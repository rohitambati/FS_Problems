...
class ShoppingPrice{
    public static void main(String[] args){
        ...
        for(int i=0;i<cart.length();i++){
            if(hm.containsKey(cart.charAt(i))){
                hm.replace(cart.charAt(i),hm.get(cart.charAt(i))+1);
            }
            else{
                hm.put(cart.charAt(i),1);
            }
        }
        int sum=0;
        for (Map.Entry<Character,Integer> entry : hm.entrySet()){  
            int index=entry.getKey();
            int count = (int)(((entry.getValue()/3)*2)+(entry.getValue()%3));
            sum+=count*prices[index-65];
        }
